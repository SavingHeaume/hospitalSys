...
public class DoctorController {
  ...
  @Autowired DrugsService drugsService;
  ...
  @Autowired SeekService seekService;
  @Value("${filepath.seekpdfpath}") private String path;
  ...
  @RequestMapping(value = "/android/getAllDocter", method = RequestMethod.GET)
  public ResponseEntity<List<Doctor>> android_getAllDoc() {
  ...
  public String seekMedicalAdvice(HttpServletRequest request, HttpSession session,
    ...
    request.setAttribute(
        "appointments", appointmentService.selectByDoctorId(doctor.getId(), patientname, time));
  ...
  public JSONObject printseek(@PathVariable Integer id, HttpSession session) {
    ...
    // createSeekInfo，第三个参数填空字符串就是生成在项目根目录里面，要是想生成在别的路径，例：D:\\
    // 就是生成在D盘根目录
